    ...
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <p class="mb-0" style="color: #38527E"><a href="{{ route('dashboard') }}" class="text-decoration-none">Pusat Halal
                    Insan Kamil</a> /
                Daftar Artikel</p>
            <a href="{{ route('kelola-artikel.create') }}" style="background-color: #38527E" class="btn mt-3 text-white"><i
                    class="fal fa-plus"></i> Tambah Artikel</a>
        </div>
                            ...
                            <tbody>
                                @foreach ($artikel as $item)
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            <a href="{{ route('kelola-artikel.edit', $item->id) }}"
                                                class="btn btn-warning btn-sm mb-1">Edit</a>
                                            <button type="button" class="btn btn-danger btn-sm mb-1"
                                                data-id="{{ $item->id }}" data-nama="{{ $item->judul }}"
                                                data-url="{{ route('kelola-artikel.destroy', $item->id) }}"
                                                onclick="confirmDelete(this)">Delete</button>

                                            <form id="delete-form-{{ $item->id }}"
                                                action="{{ route('kelola-artikel.destroy', $item->id) }}" method="POST"
                                                style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
    ...
    <script>
        function confirmDelete(button) {
            ...
            Swal.fire({
                ...
                text: `Anda akan menghapus artikel "${itemName}". Tindakan ini tidak bisa dibatalkan!`,
                icon: 'warning',
